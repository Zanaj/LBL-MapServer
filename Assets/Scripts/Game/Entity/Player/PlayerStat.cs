...
public class EntityStats : MonoBehaviour
{
    ...
    public StatType GetStatType(Stat stat)
    {
        ...
        type = index <= 5 ? StatType.Vital : type;
        type = index > 5 && index <= 20 ? StatType.Major : type;
        type = index > 20 && index <= 100 ? StatType.Minor : type;
    ...
    public void SetStat(Stat stat, float value)
    {
        ...
        if(type != StatType.Unknown)
        {
            if(type == StatType.Vital)
            {
                if (index % 2 == 0)
                {
                    Debug.LogWarning("Trying to set vital with SetStat please use SetVital. Will default to setting currentValue");
    ...
    public void DoDamageTo(EntityStats stat)
    {
        Debug.LogWarning("Attacking something!");
        if (CanDoDamage())
        {
            Vital enemyHealth = stat.GetVital(Stat.Health);
            float myMinDmg = GetStat(Stat.Min_Damage);
            float myMaxDmg = GetStat(Stat.Max_Damage);
            float myStun = GetStat(Stat.Stun);

            stat.stun = myStun;
            stat.hasBeenHit = true;
